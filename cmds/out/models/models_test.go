...
var _ = Describe("SMSConfig", func() {
	Describe("CheckInput", func() {
		...
		It("should return an error if AWS key ID is missing", func() {
			...
			Expect(err).Should(MatchError("Error: source.aws_access_key_id from stdin is either empty or missing"))
		})

		It("should return an error if AWS secret key is missing", func() {
			config.Source.AWSSecretAccessKey = ""
			err := config.CheckInput()
			Expect(err).Should(MatchError("Error: source.aws_secret_access_key from stdin is either empty or missing"))
		})

		It("should return an error if topic is missing", func() {
			config.Source.Topic = ""
			err := config.CheckInput()
			Expect(err).Should(MatchError("Error: source.topic from stdin is either empty or missing"))
		})

		It("should return an error if topic exceeds max character limit", func() {
			config.Source.Topic = "very-long-topic-1234567890abcdefg"
			err := config.CheckInput()
			Expect(err).Should(MatchError("Error: source.topic from stdin cannot exceed 10 characters"))
		})

		It("should return an error if no subscribers are provided", func() {
			config.Params.Subscribers = []string{}
			err := config.CheckInput()
			Expect(err).Should(MatchError("Error: params.subscribers from stdin is either empty or missing"))
		})

		It("should return an error if message is missing", func() {
			config.Params.Message = ""
			err := config.CheckInput()
			Expect(err).Should(MatchError("Error: params.message from stdin is either empty or missing"))
		})
